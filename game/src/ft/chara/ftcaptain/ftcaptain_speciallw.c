...
void ftCaptain_SpecialLw_ProcPhysics(GObj *fighter_gobj)
{
    ...
    if (fp->ground_or_air == GA_Ground)
    {
        fp->joint[ftParts_DefaultJoint_TopN]->rotate.z = -atan2f(fp->coll_data.ground_angle.x, fp->coll_data.ground_angle.y);
...
{
    ftStruct *fp = ftGetStruct(fighter_gobj);
    f32 rot_z = fp->joint[ftParts_DefaultJoint_TopN]->rotate.z;
    ...
    fp->joint[ftParts_DefaultJoint_TransN]->rotate.z = fp->joint[ftParts_DefaultJoint_TopN]->rotate.z;
