...
void ftFox_SpecialHi_UpdateModelPitch(GObj *fighter_gobj)
{
    ...
    fp->joint[4]->rotate.vec.f.x = (atan2f(fp->phys_info.vel_air.x, fp->phys_info.vel_air.y) * (f32)fp->lr) - HALF_PI32;
...
bool32 ftFox_SpecialHi_CheckIgnorePass(GObj *fighter_gobj)
{
    ...
    if (!(fp->coll_data.ground_flags & MPCOLL_VERTEX_CLL_PASS) || (fp->status_vars.fox.specialhi.coll_timer_unk >= 0xF))
    {
        return TRUE;
    }
    else return FALSE;
...
void ftFox_SpecialAirHi_ProcMap(GObj *fighter_gobj)
{
    ...
    if (func_ovl2_800DE758(fighter_gobj, ftFox_SpecialHi_CheckIgnorePass) != FALSE)
    {
        ...
        {
            if (lbVector_Vec3fAngleDiff(&fp->coll_data.ground_angle, &fp->phys_info.vel_air) > 1.9198622F)
            {
    ...
    coll_end:
        ...
        fp->joint[ftParts_Joint_TopN]->rotate.vec.f.y = (f32)((f32)fp->lr * HALF_PI32);
...
void ftFox_SpecialAirHi_SetStatus(GObj *fighter_gobj)
{
    ...
    ftMap_SetAir(fp);
    ftMain_SetFighterStatus(fighter_gobj, ftStatus_Fox_SpecialAirHi, fighter_gobj->anim_frame, 1.0F, FTSTATUPDATE_COLANIM_PRESERVE);
...
void ftFox_SpecialHi_DecideSetStatus(GObj *fighter_gobj)
{
    ...
    if (((stick_y + stick_x) >= FTFOX_FIREFOX_ANGLE_STICK_THRESHOLD) && !(fp->coll_data.ground_flags & MPCOLL_VERTEX_CLL_PASS))
    {
        ...
        if (!(lbVector_Vec3fAngleDiff(&fp->coll_data.ground_angle, &sp38) < HALF_PI32))
        {
...
void ftFox_SpecialHiEnd_SetStatus(GObj *fighter_gobj)
{
    ...
    ftMain_SetFighterStatus(fighter_gobj, ftStatus_Fox_SpecialHiEnd, 0.0F, 1.0F, FTSTATUPDATE_COLANIM_PRESERVE);
}
...
void ftFox_SpecialHiBound_SetStatus(GObj *fighter_gobj)
{
    ...
    ftMain_SetFighterStatus(fighter_gobj, ftStatus_Fox_SpecialHiBound, 0.0F, 1.0F, FTSTATUPDATE_COLANIM_PRESERVE);
...
void ftFox_SpecialHiStart_SetStatus(GObj *fighter_gobj)
{
    ...
    ftMain_SetFighterStatus(fighter_gobj, ftStatus_Fox_SpecialHiStart, 0.0F, 1.0F, 0U);
    ftMain_UpdateAnimCheckInterrupt(fighter_gobj);
...
void ftFox_SpecialAirHiStart_SetStatus(GObj *fighter_gobj)
{
    ...
    ftMain_SetFighterStatus(fighter_gobj, ftStatus_Fox_SpecialAirHiStart, 0.0F, 1.0F, 0U);
    ftMain_UpdateAnimCheckInterrupt(fighter_gobj);
    ftFox_SpecialHiStart_InitGravity(fp);
