...
void ftPhysics_ApplyGroundVelFriction(GObj *fighter_gobj)
{
    ...
    ftPhysics_SetGroundVelFriction(fp, dFTMainMapSurfaceFrictions[fp->coll_data.ground_flags & MPCOLL_VERTEX_MAT_MASK] * attributes->traction);
    ftPhysics_SetGroundVelTransferAir(fighter_gobj);
}
...
{
    ftPhysics_ApplyGClampTVel(fp, attributes->gravity, attributes->fall_speed_max);
}
