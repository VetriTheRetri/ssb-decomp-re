...
void grCommon_Yoster_InitGroundVars(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(gGroundStruct.yoster.clouds); i++)
    {
        ...
        omAddGObjRenderProc(map_gobj, odRenderDObjTreeForGObj, 6, 0x80000000, -1);
        func_8000F590(map_gobj, (intptr_t)&D_NF_00000100 + (uintptr_t)map_head, NULL, OMMtx_Transform_Tra, 0, 0); // Make this 0x1C to add static cloud animation
