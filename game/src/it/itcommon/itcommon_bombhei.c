...
{
    itMap_CheckLRWallProcGround(item_gobj, itBombHeiAFallSetStatus);
...
{
    return itMap_CheckMapCollideThrownLanding(item_gobj, 0.4F, 0.3F, itBombHeiGWaitSetStatus);
}
...
{
    return itMap_CheckMapProcAll(item_gobj, itBombHeiMExplodeSetStatus);
}
...
sb32 itBombHeiGWalkProcMap(GObj *item_gobj)
{
    ...
    itMap_CheckLRWallProcGround(item_gobj, itBombHeiFDropSetStatus);
...
void itBombHeiGWalkInitItemVars(GObj *item_gobj)
{
    ...
    matanim_dobj = itGetPData(ip, lItBombHeiDataStart, lItBombHeiWalkMatAnimJoint); // ((uintptr_t)ip->attributes->model_desc - (uintptr_t)&lItBombHeiDataStart) + &lItBombHeiWalkMatAnimJoint; // Linker thing

    omAddMObjAnimAll(dobj->mobj, matanim_dobj, 0.0F); // Set texture animation?
