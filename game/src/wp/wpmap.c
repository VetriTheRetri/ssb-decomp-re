...
sb32 wpMapCheckAllRebound(GObj *weapon_gobj, u32 check_flags, f32 mod_vel, Vec3f *pos) // Modify velocity based on angle of collision
{
    ...
    {
        if (func_ovl0_800C7C0C(&wp->phys_info.vel, &coll_data->lwall_angle) < 0.0F)
        {
            func_ovl0_800C7B08(&wp->phys_info.vel, &coll_data->lwall_angle);
    ...
    {
        if (func_ovl0_800C7C0C(&wp->phys_info.vel, &coll_data->rwall_angle) < 0.0F)
        {
            return_bool = TRUE;

            func_ovl0_800C7B08(&wp->phys_info.vel, &coll_data->rwall_angle);
    ...
    {
        if (func_ovl0_800C7C0C(&wp->phys_info.vel, &coll_data->ceil_angle) < 0.0F)
        {
            return_bool = TRUE;

            func_ovl0_800C7B08(&wp->phys_info.vel, &coll_data->ceil_angle);
    ...
    {
        if (func_ovl0_800C7C0C(&wp->phys_info.vel, &coll_data->ground_angle) < 0.0F)
        {
            return_bool = TRUE;
    ...
    {
        func_ovl0_800C7AE0(&wp->phys_info.vel, mod_vel);
