...
**There are no `.reloc` metadata files.** Every chain pointer is a typed
pointer expression in the committed `.c` source; `tools/fixRelocChain.py`
rebuilds each file's intern/extern relocation chains from the compiled
`.o`'s R_MIPS_32 records (cross-file targets resolve through the
`build/<v>/relocSymIndex.txt` symbol index generated after all relocData
objects compile) and byte-compares the patched output against the
original binary, failing the build on any divergence. `make verify-reloc`
runs the same comparison offline across the whole corpus
(`tools/verifyAutoReloc.py`), and `tools/triageAutoReloc.py <fid>`
explains any failing slot. Mentions of `.reloc` / `.jp.reloc` files in
the history sections below describe the retired pipeline.
...
| Auto-extracted sprite files | **96** | A committed `.spritelist` marker drives `tools/extractSpriteFile.py`, which walks the ROM binary's reloc chain, auto-discovers every Sprite + Bitmap + pixel block, and emits a full master `.c` + `.inc.c` set under `build/`. |
| **Total** | **2,134** | |
...
| 2 | `#if defined(REGION_JP)` pointer expressions | — | JP-divergent chain pointers are region-guarded typed expressions in the shared `.c`; both regions' chains derive from the per-version compile (the `.jp.reloc` override layer is retired). |
                  ...
                  └──── [3] raw-blob inline .c ─────────────────┤
                            (10 files, 50 KB)                   │
                                           ...
                                           build/<v>/src/relocData/.build/<fid>.o
                                                                ▼
                                       genRelocSymIndex.py (once all .o's compile)
                                                                ▼
                                                     objcopy .data → .vpk0.bin / .bin
                                                                ▼
                              fixRelocChain.py rebuilds chains from .o relocations
                                   and byte-compares against the original binary
                                                                ▼
...
master `.c` and companion `.inc.c` blobs are generated on the fly into
`build/<v>/src/relocData/` — no committed artifacts.
...
| Tool | Purpose |
|---|---|
| `tools/extractSpriteFile.py` | Walks the ROM reloc chain for a spritelist-driven fid, auto-discovers every Sprite via struct sanity filter, reconstructs Bitmap arrays + pixel data + embedded palettes, emits the complete inline master `.c` + texture `.inc.c` blobs. Zips sprite names in positional order with the committed `.spritelist` contents. |
| `tools/extractRelocInc.py` | Regenerates every typed block's `.inc.c` body from the decompressed ROM segment. Runs at `make extract` time. Routes emission by the declared C type (`u8` / `u16` / `u32` / `Vtx` / `Gfx` / palette) so a `#include` in a `u32 X[N] = { … }` gets u32-hex values rather than u8 bytes. Substitutes every chain-encoded pointer word inside Gfx blocks with a symbolic expression (intern targets from the file's own block layout; extern targets from the trailing file-id table + the target fid's layout) so the compiler emits a relocation per pointer slot. Uses `pygfxd` for DL macro disassembly and `n64img` for Tex PNG previews. |
...
| `tools/fixRelocChain.py` | Post-compile reloc chain rebuilder — derives every chain slot from the compiled `.o`'s R_MIPS_32 records (in-place addends give intra-block offsets; cross-file symbols resolve via `build/<v>/relocSymIndex.txt`), classifies intern vs extern by walking the original baserom binary's chains, rebuilds both chains, then byte-compares the result against the original binary and fails the build on any difference. |
| `tools/genRelocSymIndex.py` | nm sweep over every compiled relocData `.o` producing the global `symbol → (fid, offset)` index that fixRelocChain uses for cross-file extern resolution. Regenerated by the Makefile once all relocData objects compile; its mtime only advances when the symbol layout changes. |
| `tools/verifyAutoReloc.py` | Offline corpus verifier — rebuilds every fid's chains from its `.o` and byte-compares against the original binary (`make verify-reloc`). |
| `tools/triageAutoReloc.py` | Per-slot diagnosis of any divergence: classifies each failing slot as missing / extra / wrong-target and names the enclosing symbol and the expected target. |
