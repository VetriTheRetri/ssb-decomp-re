...
| 3 | `#if defined(REGION_JP)` / `REGION_US` guards inside the shared `.c` | ~42 files, **~560 guards** | Three categories: (a) per-line gameplay tuning — FTAttributes stats, WPAttributes hitbox bitfields, GR map item spawn weights, DObjDesc joint rotations, ftMotion script parameters, sprite/texture sizes, block reordering; (b) region-aware enum names (`gmFGMVoiceID` for SFX, `GMColAnimKind` for color animations) where a single name produces the correct ordinal for both regions; (c) structural layout diffs (KeyEvent array sizes, US-only sprite blocks, Lut+Tex pair reordering in StageYamabukiFile2). |
...
### JP raw-blob overrides (0 files)

All JP raw blobs have been graduated. Every file compiles from typed
C source for both US and JP. Down from 56 blobs via: chain-only
`.jp.reloc` diffs, unsized texture array fixes, ftMotion macro
