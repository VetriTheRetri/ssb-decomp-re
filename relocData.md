...
| 2 | `<id>_<Name>.jp.reloc` | 88 | JP-specific relocation chain (different extern targets / intern positions). Generated by `tools/extractJpReloc.py` which compiles the `.c` with the target region's defines so symbol offsets match the actual binary layout. Includes empty `.jp.reloc` files for raw-blob JP overrides where fixRelocChain has no patches to apply. |
| 3 | `#if defined(REGION_JP)` / `REGION_US` guards inside the shared `.c` | 46 files, **~557 guards** | Three categories: (a) per-line gameplay tuning — FTAttributes stats (size, weight, air speed), WPAttributes hitbox bitfields, GR map item spawn weights, DObjDesc joint rotations, ftMotion script parameters (hitbox damage/knockback, effect timing); (b) raw-blob `u8[]` fallback for 9 files with structural JP/US differences (see below); (c) region-aware enum names (`gmFGMVoiceID` for SFX, `GMColAnimKind` for color animations) where a single name produces the correct ordinal for both regions — `tools/replaceFGMRegionGuards.py` collapsed 128+ FGM guards and the `GMColAnimKind` enum eliminated all ColAnim ordinal guards across fighter MainMotion files. |
...
### JP raw-blob overrides (9 files)

9 files use `#if defined(REGION_JP)` to compile a single `u8 d<Name>[N]`
raw blob for JP while keeping the full US structural typing in the
`#else` branch. Down from 56 after graduating all Special2/3 files
(pure chain diffs), all Model files except LinkModel (identical data
after fixing unsized sprite texture arrays), and 5 of 7 MainMotion
files (ftMotion macro decoding + FGM/ColAnim enum replacement).

| File | Bytes | Why blob is needed |
|---|---:|---|
| `StageYamabukiFile2` | 64,016 | `extractRelocInc` parse overshoot (66160 vs 64016) |
| `SC1PIntro` | 3,616 | Pre-existing JP layout divergence |
| `MNTitle` | 3,584 | Pre-existing JP layout divergence |
| `SCStaffroll` | 46,164 | Pre-existing JP layout divergence |
| `SCExplainGraphics` | 3,472 | Pre-existing JP layout divergence |
| `SCExplainMain` | 3,024 | Pre-existing JP layout divergence |
| `SamusSpecial1` | 64 | JP binary 12 bytes larger than US |
| `CaptainMainMotion` | 7,600 | Non-FGM diffs (hitbox sizes, voice IDs, structural commands) |
| `BossMainMotion` | 2,480 | Non-FGM diffs (hitbox sizes, effect IDs) |
...
3. **Graduate the 9 remaining JP raw-blob overrides**. CaptainMainMotion
   ...
   parameter diffs. SamusSpecial1 has a 12-byte JP/US size diff.
   The 4 SC/MN scene files and StageYamabukiFile2 need investigation
