...
void efGroundInitActor(void)
{
    ...
    if ((gBattleState->gr_kind <= Gr_Kind_BattleEnd) && (gSceneData.scene_current != scMajor_Kind_1PTrainingMode) && (dEFGroundDatas[gBattleState->gr_kind].effect_param != NULL))
    {
        ...
        if (effect_gobj != NULL)
        {
            ...
            sEFGroundActor.file_head = (void*) ((uintptr_t)gGroundInfo->gr_desc[1].dobj_desc - (intptr_t)dEFGroundDatas[gBattleState->gr_kind].o_data);
