...
void ftPikachuSpecialAirHiStartSwitchStatusGround(GObj *fighter_gobj)
{
    ...
    ftMainSetFighterStatus(fighter_gobj, nFTPikachuStatusSpecialHiStart, fighter_gobj->anim_frame, 0.0F, FTPIKACHU_SPECIALHI_STATUPDATE_FLAGS);
}
...
void ftPikachuSpecialHiStartSwitchStatusAir(GObj *fighter_gobj)
{
    ...
    ftMainSetFighterStatus(fighter_gobj, nFTPikachuStatusSpecialAirHiStart, fighter_gobj->anim_frame, 0.0F, FTPIKACHU_SPECIALHI_STATUPDATE_FLAGS);
}
...
void ftPikachuSpecialAirHiProcMap(GObj *fighter_gobj)
{
    ...
    if (mpObjectProc_ProcFighterPassCliff(fighter_gobj, ftPikachuSpecialHiCheckIgnorePass) != FALSE)
    {
        ...
        else if (lbVector_Vec3fAngleDiff(&fp->coll_data.ground_angle, &fp->phys_info.vel_air) > FTPIKACHU_QUICKATTACK_HALT_ANGLE)
        {
            ftMap_SetGround(fp);
            ftPikachuSpecialHiEndSetStatus(fighter_gobj);
        }
    ...
    {
        if ((fp->coll_data.coll_mask_curr & MPCOLL_KIND_CEIL) && (lbVector_Vec3fAngleDiff(&fp->coll_data.ceil_angle, &fp->phys_info.vel_air) > FTPIKACHU_QUICKATTACK_HALT_ANGLE))
        {
            ftPikachuSpecialAirHiEndSetStatus(fighter_gobj);
        }
        if ((fp->coll_data.coll_mask_curr & MPCOLL_KIND_LWALL) && (lbVector_Vec3fAngleDiff(&fp->coll_data.lwall_angle, &fp->phys_info.vel_air) > FTPIKACHU_QUICKATTACK_HALT_ANGLE))
        {
            ftPikachuSpecialAirHiEndSetStatus(fighter_gobj);
        }
        if ((fp->coll_data.coll_mask_curr & MPCOLL_KIND_RWALL) && (lbVector_Vec3fAngleDiff(&fp->coll_data.rwall_angle, &fp->phys_info.vel_air) > FTPIKACHU_QUICKATTACK_HALT_ANGLE))
        {
            ftPikachuSpecialAirHiEndSetStatus(fighter_gobj);
        }
    }
...
{
    ftMap_SetGround(ftGetStruct(fighter_gobj));
    ftMainSetFighterStatus(fighter_gobj, nFTPikachuStatusSpecialHi, fighter_gobj->anim_frame, 0.0F, FTPIKACHU_SPECIALHI_STATUPDATE_FLAGS);
}
...
void ftPikachuSpecialHiSwitchStatusAir(GObj *fighter_gobj)
{
    ...
    ftMap_SetAir(fp);
    ftMainSetFighterStatus(fighter_gobj, nFTPikachuStatusSpecialAirHi, fighter_gobj->anim_frame, 0.0F, FTPIKACHU_SPECIALHI_STATUPDATE_FLAGS);
...
void ftPikachuSpecialHiSetStatus(GObj *fighter_gobj)
{
    ...
    if (!(sqrt_stick_range < FTPIKACHU_QUICKATTACK_STICK_RANGE_MIN) && !(fp->coll_data.ground_flags & MPCOLL_VERTEX_CLL_PASS))
    {
        ...
        if (lbVector_Vec3fAngleDiff(&fp->coll_data.ground_angle, &stick_range) < F_CST_DTOR32(90.0F)) goto block_end; /* HALF_PI32 */
...
sb32 ftPikachuSpecialHiCheckGotoSubZip(GObj *fighter_gobj)
{
    ...
    else if (fp->status_vars.pikachu.specialhi.is_subsequent_zip == FALSE)
    {
        ...
        if (lbVector_Vec3fAngleDiff(&previous_angle, &current_angle) > FTPIKACHU_QUICKATTACK_ANGLE_DIFF_MIN)
        {
            return TRUE;
        }
        else return FALSE;
...
{
    ftMap_SetGround(ftGetStruct(fighter_gobj));
    ftMainSetFighterStatus(fighter_gobj, nFTPikachuStatusSpecialHiEnd, fighter_gobj->anim_frame, 1.0F, FTPIKACHU_SPECIALHI_STATUPDATE_FLAGS);
}
...
{
    ftMap_SetAir(ftGetStruct(fighter_gobj));
    ftMainSetFighterStatus(fighter_gobj, nFTPikachuStatusSpecialAirHiEnd, fighter_gobj->anim_frame, 1.0F, FTPIKACHU_SPECIALHI_STATUPDATE_FLAGS);
}
