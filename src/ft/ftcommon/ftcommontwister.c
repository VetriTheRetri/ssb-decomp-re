...
void ftCommonTwisterProcPhysics(GObj *fighter_gobj)
{
    ...
    pos.x += (mul * bitmap_cosf(F_CLC_DTOR32(1800.0F * angle_d)));
    pos.z += (mul * bitmap_sinf(F_CLC_DTOR32(1800.0F * angle_d)));
...
void ftCommonTwisterSetStatus(GObj *fighter_gobj, GObj *tornado_gobj)
{
    ...
    ftMainSetFighterStatus(fighter_gobj, ftStatus_Common_Twister, 0.0F, 1.0F, FTSTATUPDATE_NONE_PRESERVE);
    ...
    func_800269C0_275C0(alSound_SFX_HyruleTwisterTrapped);
}
...
{
    ftStruct *fp = ftGetStruct(fighter_gobj);
    ftThrowHitDesc *tornado = (ftThrowHitDesc*) (((uintptr_t)gGroundInfo - (intptr_t)&lGRHyruleDataStart) + (intptr_t)&lGRHyruleTwisterThrowHitDesc); // Linker thing
    f32 knockback;
    s32 damage;
