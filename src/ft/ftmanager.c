...
void ftManagerAllocFighter(u32 data_flags, s32 allocs_num)
{
    ...
    sFTManagerStructsAllocBuf = sFTManagerStructsAllocFree = syTasklogMalloc(sizeof(ftStruct) * allocs_num, 0x8);
    ...
    sFTManagerPartsAllocFree = sFTManagerPartsAllocBuf = syTasklogMalloc(sizeof(ftParts) * allocs_num * FTPARTS_JOINT_NUM_MAX, 0x8);
    ...
    gFTManagerCommonFile = lbRelocGetFileExternHeap((u32)&D_NF_000000A3, syTasklogMalloc(lbRelocGetFileSize((u32)&D_NF_000000A3), 0x10));

    lbRelocGetFileExternHeap((u32)&D_NF_000000C9, syTasklogMalloc(lbRelocGetFileSize((u32)&D_NF_000000C9), 0x10));
...
void ftManagerSetupFilesMainKind(s32 ft_kind)
{
    ...
    *ft_data->p_file_main = lbRelocGetFileExternHeap(ft_data->file_main_id, syTasklogMalloc(lbRelocGetFileSize(ft_data->file_main_id), 0x10));
...
void* ftManagerAllocAnimHeapKind(s32 ft_kind)
{
    ...
    return syTasklogMalloc(ft_data->file_anim_size, 0x10);
}
...
GObj* ftManagerMakeFighter(ftCreateDesc *ft_desc) // Create fighter
{
    ...
    fighter_gobj = gcMakeGObjSPAfter(nOMObjCommonKindFighter, NULL, nOMObjCommonLinkIDFighter, GOBJ_LINKORDER_DEFAULT);

    gcAddGObjDisplay(fighter_gobj, ft_desc->proc_display, FTRENDER_DLLINK_DEFAULT, GOBJ_DLLINKORDER_DEFAULT, GOBJ_CAMTAG_DEFAULT);
    ...
    if (fp->status_info.pl_kind != nFTPlayerKindDemo)
    {
        gcAddGObjProcess(fighter_gobj, ftMainProcInterruptMain, nOMObjProcessKindProc, 5);
        gcAddGObjProcess(fighter_gobj, ftMainProcPhysicsMapDefault, nOMObjProcessKindProc, 4);
        gcAddGObjProcess(fighter_gobj, ftMainProcPhysicsMapCapture, nOMObjProcessKindProc, 3);
        gcAddGObjProcess(fighter_gobj, ftMainProcSearchAllCatch, nOMObjProcessKindProc, 2);
        gcAddGObjProcess(fighter_gobj, ftMainProcSearchAllHit, nOMObjProcessKindProc, 1);
        gcAddGObjProcess(fighter_gobj, ftMainProcUpdateMain, nOMObjProcessKindProc, 0);
    }
    else gcAddGObjProcess(fighter_gobj, scSubsysFighterProcUpdate, nOMObjProcessKindProc, 5);
