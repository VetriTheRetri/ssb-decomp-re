...
void grYamabukiGateUpdateOpen(void)
{
    ...
    else if (gGRCommonStruct.yamabuki.gate_noentry == FALSE)
    {
        ...
        gGRCommonStruct.yamabuki.gate_pos.x = DObjGetStruct(gGRCommonStruct.yamabuki.monster_gobj)->translate.vec.f.x - ip->coll_data.obj_coll.width;
        gGRCommonStruct.yamabuki.gate_pos.y = gMPCollisionYakumonoDObjs->yakumono_dobj[3]->translate.vec.f.y;
...
void grYamabukiMakeGate(void)
{
    ...
    gcAddGObjDisplay(gate_gobj, gcDrawDObjTreeDLLinksForGObj, 6, GOBJ_PRIORITY_DEFAULT, -1);
    gcSetupCustomDObjs
    (
        gate_gobj, 
        (DObjDesc*) 
