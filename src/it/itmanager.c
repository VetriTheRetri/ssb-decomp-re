...
void itManagerInitItems(void) // Many linker things here
{
    ...
    gITManagerStructsAllocFree = ip = syTaskMalloc(sizeof(itStruct) * ITEM_ALLOC_MAX, 0x8);
    ...
    gITManagerFileData = (void*)lbRelocGetFileExternHeap(&D_NF_000000FB, syTaskMalloc(lbRelocGetFileSize(&D_NF_000000FB), 0x10));
    ...
    if (gBattleState->item_appearance_rate != nSCBattleItemSwitchNone)
    {
        if (gBattleState->item_toggles != 0)
        {
            if (gMPCollisionGroundData->item_weights != NULL)
            {
                ...
                gITManagerSpawnActor.item_mapobjs = (u8*)syTaskMalloc(item_mapobj_count * sizeof(*gITManagerSpawnActor.item_mapobjs), 0);
                ...
                gITManagerSpawnActor.weights.item_kinds = (u8*)syTaskMalloc(j * sizeof(*gITManagerSpawnActor.weights.item_kinds), 0x0);
                gITManagerSpawnActor.weights.item_totals = (u16*)syTaskMalloc(j * sizeof(*gITManagerSpawnActor.weights.item_totals), 0x2);
...
void itManagerSetupContainerDrops(void)
{
    ...
    if ((gBattleState->item_appearance_rate != nSCBattleItemSwitchNone) && (gBattleState->item_toggles != 0) && (gMPCollisionGroundData->item_weights != NULL))
    {
        ...
        if (item_count != 0)
        {
            ...
            gITManagerRandomWeights.item_kinds = (u8*)syTaskMalloc(j * sizeof(*gITManagerRandomWeights.item_kinds), 0x0);
            gITManagerRandomWeights.item_totals = (u16*)syTaskMalloc(j * sizeof(*gITManagerRandomWeights.item_totals), 0x2);
