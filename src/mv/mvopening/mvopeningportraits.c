...
// 0x80131FC4
void mvOpeningPortraitsCoverFuncDisplay(GObj *gobj)
{
...
void mvOpeningPortraitsMakeCover(void)
{
	...
	gcAddGObjDisplay(gobj, mvOpeningPortraitsCoverFuncDisplay, 28, GOBJ_PRIORITY_DEFAULT, ~0);
...
void mvOpeningPortraitsFuncStart(void)
{
	...
	(
		dMVOpeningPortraitsFileIDs,
		ARRAY_COUNT(dMVOpeningPortraitsFileIDs),
		sMVOpeningPortraitsFiles,
		syTaskmanMalloc
		(
			lbRelocGetAllocSize
			(
...
{
	dMVOpeningPortraitsVideoSetup.zbuffer = syVideoGetZBuffer(6400);
	syVideoInit(&dMVOpeningPortraitsVideoSetup);

	dMVOpeningPortraitsTaskmanSetup.buffer_setup.arena_size = (size_t) ((uintptr_t)&ovl1_VRAM - (uintptr_t)&ovl35_BSS_END);
	syTaskmanRun(&dMVOpeningPortraitsTaskmanSetup);
}
