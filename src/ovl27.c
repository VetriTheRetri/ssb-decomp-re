...
// 0x80138EE8
MNPlayersSlot1PGame gMN1PPanel;
...
// 0x80132384
void mn1PSelectFighterPuck(s32 player, s32 select_button)
{
...
// 0x8013243C
f32 mn1PGetNextPortraitX(s32 portrait, f32 current_pos_x)
{
...
// 0x80132550
sb32 mn1PCheckFighterCrossed(s32 fkind)
{
	return FALSE;
}

// 0x8013255C
void mn1PPortraitProcUpdate(GObj *portrait_gobj)
{
	f32 new_pos_x = mn1PGetNextPortraitX(portrait_gobj->user_data.s, SObjGetStruct(portrait_gobj)->pos.x);
...
// 0x801325D8
void mn1PSetPortraitWallpaperPosition(SObj *sobj, s32 portrait)
{
...
// 0x80132634
void mn1PPortraitAddCross(GObj* portrait_gobj, s32 portrait)
{
	...
	portrait_sobj = lbCommonMakeSObjForGObj(portrait_gobj, lbRelocGetFileData(void*, gMN1PFiles[4], xbox_image_offset));
...
// 0x801326C8
sb32 mn1PCheckFighterLocked(s32 fkind)
{
	switch (fkind)
	{
		case nFTKindNess:
			return (gMN1PFighterMask & (1 << nFTKindNess)) ? FALSE : TRUE;

		case nFTKindPurin:
			return (gMN1PFighterMask & (1 << nFTKindPurin)) ? FALSE : TRUE;

		case nFTKindCaptain:
			return (gMN1PFighterMask & (1 << nFTKindCaptain)) ? FALSE : TRUE;

		case nFTKindLuigi:
			return (gMN1PFighterMask & (1 << nFTKindLuigi)) ? FALSE : TRUE;
	}
	return FALSE;
}
...
// 0x8013283C
void mn1PPortraitProcDisplay(GObj *portrait_gobj)
{
...
// 0x801328FC
void mn1PMakePortraitShadow(s32 portrait)
{
	GObj* texture_gobj;
	SObj* texture_sobj;
	...
	gcAddGObjProcess(texture_gobj, mn1PPortraitProcUpdate, 1, 1);

	texture_sobj = lbCommonMakeSObjForGObj(texture_gobj, lbRelocGetFileData(void*, gMN1PFiles[4], &lMNPlayersPortraitsWallpaperSprite));
	texture_sobj->pos.x = (f32) (((portrait >= 6 ? portrait - 6 : portrait) * 0x2D) + 0x19);
	texture_sobj->pos.y = (f32) (((portrait >= 6 ? 1 : 0) * 0x2B) + 0x24);

	mn1PSetPortraitWallpaperPosition(texture_sobj, portrait);
	texture_gobj->user_data.s = portrait;

	// portrait
	texture_gobj = gcMakeGObjSPAfter(0U, NULL, 0x12U, 0x80000000U);
	gcAddGObjDisplay(texture_gobj, mn1PPortraitProcDisplay, 0x1BU, GOBJ_PRIORITY_DEFAULT, ~0);
	gcAddGObjProcess(texture_gobj, mn1PPortraitProcUpdate, 1, 1);
	...
	texture_sobj = lbCommonMakeSObjForGObj(texture_gobj, lbRelocGetFileData(void*, gMN1PFiles[4], &lMNPlayersPortraitsQuestionSprite));
...
// 0x80132BA4
void mn1PMakePortrait(s32 portrait)
{
	...
	if (mn1PCheckFighterLocked(mn1PGetFighterKind(portrait)))
		mn1PMakePortraitShadow(portrait);
	else
	{
		// portrait bg (fire)
		...
		gcAddGObjProcess(portrait_bg_gobj, mn1PPortraitProcUpdate, 1, 1);

		texture_sobj = lbCommonMakeSObjForGObj(portrait_bg_gobj, lbRelocGetFileData(void*, gMN1PFiles[4], &lMNPlayersPortraitsWallpaperSprite));
		mn1PSetPortraitWallpaperPosition(texture_sobj, portrait);
		...
		gcAddGObjProcess(portrait_gobj, mn1PPortraitProcUpdate, 1, 1);
		...
		if (mn1PCheckFighterCrossed(mn1PGetFighterKind(portrait)))
			mn1PPortraitAddCross(portrait_gobj, portrait);
...
// 0x80132D60
void mn1PMakePortraits()
{
	s32 portrait;

	for (portrait = 0; portrait < 12; portrait++)
		mn1PMakePortrait(portrait);
}
...
// 0x80132F1C
void mn1PMakePortraitCamera(void)
{
...
// 0x80132FBC
void mn1PMakePortraitWallpaperCamera()
{
...
// 0x8013305C
void mn1PMakePortraitFlashCamera()
{
...
void mn1PMakeGate(s32 player)
{
	...
	// create panel
	gobj = lbCommonMakeSpriteGObj(0, NULL, 0x16, 0x80000000, lbCommonDrawSObjAttr, 0x1C, GOBJ_PRIORITY_DEFAULT, ~0, lbRelocGetFileData(void*, gMN1PFiles[5], &lMNPlayers1PModeGateWallpaperSprite), 1, NULL, 1);
...
void mn1PDrawPickerOptionsTitleAndBack()
{
	...
	// Back image
	picker_sobj = lbCommonMakeSObjForGObj(picker_gobj, lbRelocGetFileData(void*, gMN1PFiles[0], &lMNPlayersCommonBackSprite));
...
void mn1PDrawHighscore()
{
	...
	s32 best_difficulty;
	s32 fkind = mn1PGetForcePuckFighterKind();
...
void mn1PUpdateCursor(GObj* cursor_gobj, s32 player, s32 cursor_status)
{
	...
	cursor_sobj = lbCommonMakeSObjForGObj(cursor_gobj, lbRelocGetFileData(void*, gMN1PFiles[0], cursor_offsets[cursor_status]));
...
// 0x8013565C
sb32 mn1PCheckBackInRange(GObj* cursor_gobj)
{
	f32 current_x, current_y;
	s32 range_check;
	SObj* cursor_sobj;
...
sb32 mn1PCheckPuckInRange(GObj* cursor_gobj, s32 cursor_player, s32 player)
{
	...
	SObj* puck_sobj = SObjGetStruct(gMN1PPanel.puck);
...
void mn1PDestroyPortraitFlash(s32 player)
{
	...
	flash_gobj = gMN1PPanel.flash;
	if (flash_gobj != NULL)
	{
		gMN1PPanel.flash = NULL;
		gcEjectGObj(flash_gobj);
	}
}

// 0x8013595C
void mn1PPortraitFlash(GObj* flash_gobj)
{
	s32 duration = 16;
...
void mn1PMakePortraitFlash(s32 player)
{
	...
	gcAddGObjProcess(flash_gobj, mn1PPortraitFlash, 0, 1);

	flash_sobj = lbCommonMakeSObjForGObj(flash_gobj, lbRelocGetFileData(void*, gMN1PFiles[4], &lMNPlayersPortraitsFlashSprite));
...
sb32 mn1PSelectChar(GObj* cursor_gobj, s32 player, s32 arg2, s32 select_button)
{
	...
	{
		mn1PSelectFighterPuck(player, select_button);
		gMN1PPanel.recall_end_tic = gMN1PTotalTimeTics + 0x1E;
		return TRUE;
	}
...
void mn1PUpdateCursorGrabDLLinks(s32 player, s32 puck_id)
{
	...
	gcMoveGObjDL(gMN1PPanel.puck, 0x1E, display_orders[player] + 1);
}
...
void mn1PUpdateCursorPlacementDLLinks(s32 player)
{
	...
	gcMoveGObjDL(gMN1PPanel.puck, 0x1F, unheld_orders[player]);
}
...
// 0x80136050
s32 mn1PGetPuckFighterKind(s32 player)
{
	SObj* puck_sobj = SObjGetStruct(gMN1PPanel.puck);
	s32 current_y = (s32) puck_sobj->pos.x + 13;
	s32 current_x = (s32) puck_sobj->pos.y + 12;
	...
	if (is_within_bounds)
	{
		...
		if (is_within_bounds)
		{
			...
			if ((mn1PCheckFighterCrossed(fkind)) || (mn1PCheckFighterLocked(fkind)))
				return nFTKindNull;
...
// 0x801361C8
void mn1PAdjustCursor(GObj* cursor_gobj, s32 player)
{
	...
	if (gMN1PPanel.is_cursor_adjusting)
	{
		...
		{
			gMN1PPanel.is_cursor_adjusting = FALSE;
		}

		SObjGetNext(SObjGetStruct(cursor_gobj))->pos.x = (f32) ((f32) coords[gMN1PPanel.cursor_status].x + SObjGetStruct(cursor_gobj)->pos.x);
		SObjGetNext(SObjGetStruct(cursor_gobj))->pos.y = (f32) ((f32) coords[gMN1PPanel.cursor_status].y + SObjGetStruct(cursor_gobj)->pos.y);
	}
	else if (!gMN1PPanel.is_recalling)
	{
		...
		if (is_within_bounds != 0)
		{
			...
			if (is_within_bounds != 0)
			{
				...
				SObjGetNext(SObjGetStruct(cursor_gobj))->pos.x = SObjGetStruct(cursor_gobj)->pos.x + coords[gMN1PPanel.cursor_status].x;
			}
		}
		...
		if (is_within_bounds != 0)
		{
			...
			if (is_within_bounds != 0)
			{
				...
				SObjGetNext(SObjGetStruct(cursor_gobj))->pos.y = SObjGetStruct(cursor_gobj)->pos.y + coords[gMN1PPanel.cursor_status].y;
			}
		}
	}
}
...
// 0x80136B44
void mn1PCostumeSync(s32 arg0, s32 select_button)
{
...
// 0x80136BAC
sb32 mn1PCheckManFighterSelected(s32 arg0)
{
...
// 0x80136BD4
void mn1PRecallPuck(s32 player)
{
	gMN1PPanel.unk_0x88 = FALSE;
	gMN1PPanel.is_selected = FALSE;
	gMN1PPanel.is_recalling = TRUE;
	gMN1PPanel.recall_tics = 0;
	gMN1PPanel.recall_start_x = SObjGetStruct(gMN1PPanel.puck)->pos.x;
	gMN1PPanel.recall_start_y = SObjGetStruct(gMN1PPanel.puck)->pos.y;
...
// 0x80136D04
void mn1PDetectBack(s32 player)
{
...
// 0x80136D58
void mn1PCursorProcUpdate(GObj* cursor_gobj)
{
	...
	mn1PAdjustCursor(cursor_gobj, player);
...
// 0x80137034
void mn1PCenterPuckInPortrait(GObj* puck_gobj, s32 fkind)
{
	s32 portrait = mn1PGetPortrait(fkind);
...
// 0x801370EC
void mn1PMovePuck(s32 player)
{
	SObjGetStruct(gMN1PPanel.puck)->pos.x += gMN1PPanel.puck_vel_x;
	SObjGetStruct(gMN1PPanel.puck)->pos.y += gMN1PPanel.puck_vel_y;
}

// 0x8013712C
void mn1PPuckProcUpdate(GObj* puck_gobj)
{
	s32 fkind;
	...
	if ((!gMN1PPanel.is_selected)
		&& (gMN1PPanel.holder_player != 4)) {
		if (!gMN1PPanel.is_cursor_adjusting) {
			SObjGetStruct(puck_gobj)->pos.x = (f32) (SObjGetStruct(gMN1PPanel.cursor)->pos.x + 11.0F);
			SObjGetStruct(puck_gobj)->pos.y = (f32) (SObjGetStruct(gMN1PPanel.cursor)->pos.y + -14.0F);
		}
	}
	else
		mn1PMovePuck(player);

	fkind = mn1PGetPuckFighterKind(player);
...
// 0x80137448
void mn1PMakeCursor(s32 player)
{
	GObj* cursor_gobj;
	s32 unused;
	...
	cursor_gobj = lbCommonMakeSpriteGObj(0, NULL, 0x13, 0x80000000, lbCommonDrawSObjAttr, 0x1E, cursor_starting_display_orders[player], -1, lbRelocGetFileData(void*, gMN1PFiles[0], &FILE_011_CURSOR_POINTER_IMAGE_OFFSET), 1, mn1PCursorProcUpdate, 2);
	...
	mn1PUpdateCursor(cursor_gobj, player, 0);
}

// 0x80137590
void mn1PMakePuck(s32 player)
{
	GObj* puck_gobj;
	MNPlayersSlot1PGame* pslot;
	...
	puck_gobj = lbCommonMakeSpriteGObj(0, NULL, 0x14, 0x80000000, lbCommonDrawSObjAttr, 0x1F, orders1[player], -1, lbRelocGetFileData(void*, gMN1PFiles[0], offsets[player]), 1, mn1PPuckProcUpdate, 1);

	puck_gobj->user_data.s = player;
	gMN1PPanel.puck = puck_gobj;
	...
	if (gMN1PPanel.fkind == nFTKindNull)
	{
		...
		mn1PCenterPuckInPortrait(puck_gobj, gMN1PPanel.fkind);
...
// 0x801376F8
void mn1PPuckAdjustPortraitEdge(s32 player)
{
	s32 portrait = mn1PGetPortrait(gMN1PPanel.fkind);
	f32 portrait_edge_x = ((portrait >= 6) ? portrait - 6 : portrait) * 45 + 25;
	f32 portrait_edge_y = ((portrait >= 6) ? 1 : 0) * 43 + 36;
	f32 new_pos_x = SObjGetStruct(gMN1PPanel.puck)->pos.x + gMN1PPanel.puck_vel_x + 13.0F;
	f32 new_pos_y = SObjGetStruct(gMN1PPanel.puck)->pos.y + gMN1PPanel.puck_vel_y + 12.0F;

	if (new_pos_x < (portrait_edge_x + 5.0F))
	{
		gMN1PPanel.puck_vel_x = ((portrait_edge_x + 5.0F) - new_pos_x) / 10.0F;
	}
	if (((portrait_edge_x + 45.0F) - 5.0F) < new_pos_x)
	{
		gMN1PPanel.puck_vel_x = ((new_pos_x - ((portrait_edge_x + 45.0F) - 5.0F)) * -1.0F) / 10.0F;
	}
	if (new_pos_y < (portrait_edge_y + 5.0F))
	{
		gMN1PPanel.puck_vel_y = ((portrait_edge_y + 5.0F) - new_pos_y) / 10.0F;
	}
	if (((portrait_edge_y + 43.0F) - 5.0F) < new_pos_y)
	{
		gMN1PPanel.puck_vel_y = ((new_pos_y - ((portrait_edge_y + 43.0F) - 5.0F)) * -1.0F) / 10.0F;
	}
}

// 0x801378A8
void mn1PPuckAdjustPlaced(s32 player)
{
	mn1PPuckAdjustPortraitEdge(player);
}

// 0x801378C8
void mn1PPuckAdjustRecall(s32 player)
{
	f32 new_y_velocity, new_x_velocity;

	gMN1PPanel.recall_tics += 1;

	if (gMN1PPanel.recall_tics < 11)
	{
		new_x_velocity = (gMN1PPanel.recall_end_x - gMN1PPanel.recall_start_x) / 10.0F;

		if (gMN1PPanel.recall_tics < 6)
		{
		...
		gMN1PPanel.puck_vel_x = new_x_velocity;
		gMN1PPanel.puck_vel_y = new_y_velocity;
	}
	...
	if (gMN1PPanel.recall_tics == 30)
		gMN1PPanel.is_recalling = FALSE;
}

// 0x8013799C
void mn1PPuckAdjustProcUpdate(s32 player)
{
	if (gMN1PPanel.is_recalling)
		mn1PPuckAdjustRecall(gMN1PHumanPanelPort);

	if (gMN1PPanel.is_selected)
		mn1PPuckAdjustPlaced(0);
}

// 0x801379E8
void mn1PMakePuckAdjust()
{
	gcAddGObjProcess(gcMakeGObjSPAfter(0U, NULL, 0x18U, 0x80000000U), mn1PPuckAdjustProcUpdate, 1, 1);
}

// 0x80137A2C
void mn1PSpotlightProcUpdate(GObj* white_circle_gobj)
{
	f32 sizes[12] = {
...
// 0x80137B04
void mn1PMakeSpotlight()
{
	GObj* white_circle_gobj;

	white_circle_gobj = gcMakeGObjSPAfter(0U, NULL, 0x15U, 0x80000000U);

	gcSetupCommonDObjs(white_circle_gobj, lbRelocGetFileData(void*, gMN1PFiles[10], &lMNPlayersSpotlightDObjDesc), 0);

	gcAddGObjDisplay(white_circle_gobj, gcDrawDObjTreeDLLinksForGObj, 9U, GOBJ_PRIORITY_DEFAULT, ~0);

	gcAddMObjAll(white_circle_gobj, lbRelocGetFileData(void*, gMN1PFiles[10], &lMNPlayersSpotlightMObjSub));

	gcAddGObjProcess(white_circle_gobj, mn1PSpotlightProcUpdate, 1, 1);
...
void mn1PReadyProcUpdate(GObj* gobj)
{
	if (mn1PCheckReady())
	{
		...
		if (gMN1PReadyBlinkWait == 40)
		{
			gMN1PReadyBlinkWait = 0;
		}

		gobj->flags = (gMN1PReadyBlinkWait < 30) ? GOBJ_FLAG_NONE : GOBJ_FLAG_HIDDEN;
	}
	else
	{
...
void mn1PMakeReady()
{
	...
	sobj = lbCommonMakeSObjForGObj(gobj, lbRelocGetFileData(void*, gMN1PFiles[0], &lMNPlayersCommonReadyBannerSprite));
	sobj->sprite.attr &= ~SP_FASTCOPY;
	sobj->sprite.attr |= SP_TRANSPARENT;
	sobj->envcolor.r = 0;
	sobj->envcolor.g = 0;
	...
	sobj = lbCommonMakeSObjForGObj(gobj, lbRelocGetFileData(void*, gMN1PFiles[0], &lMNPlayersCommonReadySprite));
	sobj->sprite.attr &= ~SP_FASTCOPY;
	sobj->sprite.attr |= SP_TRANSPARENT;
	...
	// "Press"
	sobj = lbCommonMakeSObjForGObj(gobj, lbRelocGetFileData(void*, gMN1PFiles[0], &lMNPlayersCommonPressSprite));
	sobj->sprite.attr &= ~SP_FASTCOPY;
	sobj->sprite.attr |= SP_TRANSPARENT;
	...
	// "Start"
	sobj = lbCommonMakeSObjForGObj(gobj, lbRelocGetFileData(void*, gMN1PFiles[0], &lMNPlayersCommonStartSprite));
	sobj->sprite.attr &= ~SP_FASTCOPY;
	sobj->sprite.attr |= SP_TRANSPARENT;
	sobj->sprite.red = 0xFF;
	sobj->sprite.green = 0x56;
...
// 0x80138150
void mn1PInitPlayer(s32 player)
{
	gMN1PPanel.flash = NULL;
...
void mn1PLoadMatchInfo()
{
	...
	mn1PInitPlayer(gMN1PHumanPanelPort);

	gMN1PPanel.recall_end_tic = 0;
	gMN1PFighterMask = gSCManagerBackupData.fighter_mask;
}

// 0x801382C8
void mn1PInitSlot(s32 player)
{
	mn1PMakeCursor(player);
	mn1PMakePuck(player);
	mn1PMakeGate(player);
...
void mn1PGamePlayersFuncStart(void)
{
	...
	mn1PMakeWallpaper();
	mn1PMakePortraits();
	mn1PInitSlot(gMN1PHumanPanelPort);
