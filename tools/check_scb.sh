#!/bin/bash
# Generate src/particles/<name>_scb.c from a baserom range, build, and
# verify the .rodata bytes match the baserom slice.
#
# Usage: tools/check_scb.sh <name> <ucase> <rom_lo> <rom_hi>
set -euo pipefail
name="$1"; ucase="$2"; rom_lo="$3"; rom_hi="$4"
out="src/particles/${name}_scb.c"

python3 tools/decode_scb.py "$name" "$rom_lo" "$rom_hi" --ucase "$ucase" > "$out"
touch "$out"
