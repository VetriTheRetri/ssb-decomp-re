#!/usr/bin/env python3
"""Extract the 16x16 RGBA5551 mouse-cursor texture from ovl8_8's .data
section (`D_ovl8_80387CA8`).

This is the arrow cursor the debug menu draws: `D_ovl8_80387EA8` (Bitmap)
points at it and `D_ovl8_80387EB8` (Sprite) declares it 16x16
G_IM_FMT_RGBA / G_IM_SIZ_16b. `func_ovl8_8037BCB8` is what puts it on
screen, tagging the node 'Arow'.

Reads the baserom at the version-specific ROM offset, extracts the
512-byte texture, and writes:
  - assets/<ver>/ovl8/ovl8_8_arrow.rgba16.bin  (raw N64 pixels)
  - assets/<ver>/ovl8/ovl8_8_arrow.rgba16.png  (RGBA preview)
  - build/<ver>/src/ovl8/ovl8_8_arrow.rgba16.inc.c  (u16 hex for #include)
...
# ROM offsets: ovl8_8 .data start (from the splat yaml) + 8, since the
# section opens with the two words of D_ovl8_80387CA0 before the texture.
ROM_OFFSETS = {
    "us": 0x1A44F0 + 8,  # 0x1A44F8
    "jp": 0x1A4770 + 8,  # 0x1A4778
}
