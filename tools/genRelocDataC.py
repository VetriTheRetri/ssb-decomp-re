...
def generate(file_id, data, file_name, entries, reloc_map, csv_entry,
    ...
    asset_dir = os.path.join(PROJECT_DIR, "build", "us", "src", "relocData", file_subdir)
    ...
    def emit_data_block(start, end, name_suffix):
        ...
        if extract_data:
            ...
            inc_dir = os.path.join(PROJECT_DIR, "build", "us", "src", "relocData",
                                    sub_dir_name)
    ...
    def emit_displaylist_block(offset, name, next_offset):
        """Emit an F3DEX display list as a wrapper + companion .dl.inc.c.

        The display list runs from `offset` to the first `gsSPEndDisplayList`
        (cmd byte 0xDF), inclusive of the terminator's 8 bytes. The
        committed wrapper declares `Gfx X[N] = { #include <...> };` and the
        raw `{ { w0, w1 } },` command stream lives in
        `build/src/relocData/<Sub>/<name>.dl.inc.c`, regenerated by
        extractRelocInc at every `make extract`. Chain-encoded pointers in
        w1 still become real addresses via fixRelocChain at link time.
        ...
        cmd_count = (end - offset) // GFX_SIZE
        sub_dir_name = os.path.basename(block_dir.rstrip('/'))
        inc_name = f"{name_suffix}.dl.inc.c"

        # Still register reloc chain entries for pointer-carrying commands
        # so fixRelocChain patches the right offsets at link time.
        ...
        block_lines = [
            f"/* DisplayList: {name} @ 0x{offset:X} ({end - offset} bytes, {cmd_count} cmds) */",
            '',
            '#include "relocdata_types.h"',
            '',
            f"Gfx {var_name}[{cmd_count}] = {{",
            f"\t#include <{sub_dir_name}/{inc_name}>",
            "};",
        ]
